...
#define HS_AX25_MIN_PACKET_LEN 18
#define HS_AX25_MAX_PACKET_LEN (7 + 7 + 7 * HS_AX25_MAX_REPEATER_COUNT + 2 + HS_AX25_MAX_INFO_LEN + 2)
...
{
    hs_byte buffer[HS_AX25_MAX_PACKET_LEN];
    int buffer_pos;
 ...
 * @param packet Pointer to the hs_ax25_packet_t structure where the packet will be stored.
 * @param bit The bit to be processed.
 *
 * @return 1 if a packet is ready to be read, 0 otherwise.
 */
